---
name: "工具收錄申請 / Tool submission"
about: "提交 AI 工具與官方證據，供問問貓查核 / Suggest an AI tool with official sources"
title: "[工具投稿] "
labels: ""
assignees: ""
---

<!-- Issues are public. Do not post private contact details, credentials or customer data. -->
<!-- 本 Issue 公開。請勿張貼私人聯絡資料、帳號密碼或客戶資料；需要私下聯繫請寄 info@ampm-aiops.com。 -->
<!-- 本站不提供付費收錄、付費優先查核或贊助排名。收錄與評分一律依官方證據判定。 -->
<!-- We do not offer paid listings, paid priority review or sponsored rankings. Inclusion and scoring are based solely on official evidence. -->

## 工具基本資料 / Tool details
- 工具名稱 / Name:
- 主要用途 / Main use:
- 官方首頁 / Official homepage:
- 官方定價頁 / Official pricing URL:
- 與工具的關係 / Your relationship to the tool:

## 免費額度 / Free tier
- 是否有免費方案 / Free plan available:
- 額度、單位與重設週期 / Amount, unit and reset period:
- 新用戶、綁卡、浮水印等限制 / Trial, card or watermark restrictions:
- 官方來源與查閱日期 / Official source and date accessed:

## 商用授權 / Commercial use
- 允許、附條件、不允許或官方未公布 / Allowed, conditional, prohibited or unpublished:
- 官方條款網址與相關段落 / Official terms URL and relevant passage:

## 付費方案（如有） / Paid plans, if applicable
- 方案名稱、幣別與含稅狀況 / Plan, currency and tax status:
- 月繳牌價 / Monthly billing price:
- 年繳總額（非月均價） / Annual total, not monthly equivalent:
- 促銷原價、資格、截止日與時區 / Promotion, eligibility, expiry and timezone:
- 官方來源 / Official source:

## 交叉查核來源 / Supporting evidence
- 官方 FAQ、說明中心或條款 / Official secondary source:
- 獨立第三方來源（選填） / Independent third-party source, optional:
- 來源有無矛盾或未知項目 / Conflicts or unknowns:

## 聯絡 / Contact
- 可在此公開 Issue 回覆 / Reply in this public issue: 是 / Yes
- 需要私下聯繫請寄 info@ampm-aiops.com，並附 Issue 網址。（本站不接受付費收錄或優先查核委託）

## 申請確認 / Acknowledgements
- [ ] 我已搜尋既有 Issue 與工具清單，避免重複提交。 / I checked existing issues and listings.
- [ ] 我未把年繳月均價當成月繳牌價；官方未公布的項目已標示未知。 / Unknowns and billing periods are explicit.
- [ ] 我理解提交不保證收錄，收錄與評分一律依官方證據判定。 / Submission does not guarantee listing; inclusion and scoring are based solely on official evidence.
- [ ] 我未提交密碼、API 金鑰或非公開個資。 / No credentials or private personal data are included.
